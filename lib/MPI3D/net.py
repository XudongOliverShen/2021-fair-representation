...
def ResNet34(dim_out=32):
    return ResNet(BasicBlock, [3,4,6,3], dim_out)
...
def ResNet34_transposed(dim_in=32+1):
    return ResNet_transposed(BasicBlock_transposed, [3,4,6,3], dim_in)
...
class DenseBlock_deeper(nn.Module):
    def __init__(self, dim_in=166, dim_hidden=256, dim_out=4, sigmoid=True):
        super(DenseBlock_deeper, self).__init__()
        self.dim_in = dim_in
        self.dim_hidden = dim_hidden
        self.fc1 = nn.Linear(in_features=dim_in, out_features=dim_hidden, bias=False)
        self.bn1 = nn.BatchNorm1d(1)
        ...
        self.fc5 = nn.Linear(in_features=dim_hidden, out_features=dim_hidden, bias=False)
        self.bn5 = nn.BatchNorm1d(1)
        self.fc6 = nn.Linear(in_features=dim_hidden, out_features=dim_hidden, bias=False)
        self.bn6 = nn.BatchNorm1d(1)
        self.fc7 = nn.Linear(in_features=dim_hidden, out_features=dim_hidden, bias=False)
        self.bn7 = nn.BatchNorm1d(1)
        self.fc8 = nn.Linear(in_features=dim_hidden, out_features=dim_hidden, bias=False)
        self.bn8 = nn.BatchNorm1d(1)
        self.fc9 = nn.Linear(in_features=dim_hidden, out_features=dim_hidden, bias=False)
        self.bn9 = nn.BatchNorm1d(1)
        self.fc10 = nn.Linear(in_features=dim_hidden, out_features=dim_hidden, bias=False)
        ...
        self.h_o6 = nn.Sequential(
            nn.Linear(dim_in, dim_hidden, bias=False),
            nn.BatchNorm1d(1)
        )
        self.h_o7 = nn.Sequential(
            nn.Linear(dim_in, dim_hidden, bias=False),
            nn.BatchNorm1d(1)
        )
        self.h_o8 = nn.Sequential(
            nn.Linear(dim_in, dim_hidden, bias=False),
            nn.BatchNorm1d(1)
        )
        self.h_o9 = nn.Sequential(
            nn.Linear(dim_in, dim_hidden, bias=False),
            nn.BatchNorm1d(1)
        )
        self.h_o10 = nn.Sequential(
            nn.Linear(dim_in, dim_hidden, bias=False),
            nn.BatchNorm1d(1)
        )
    ...
    def forward(self, h):
        ...
        o4 = self.relu( self.bn4(self.fc4(o3)) ) + self.h_o4(h)
        o5 = self.relu( self.bn5(self.fc5(o4)) ) + self.h_o5(h)
        o6 = self.relu( self.bn6(self.fc6(o5)) ) + self.h_o6(h)
        o7 = self.relu( self.bn7(self.fc7(o6)) ) + self.h_o7(h)
        o8 = self.relu( self.bn8(self.fc8(o7)) ) + self.h_o8(h)
        o9 = self.relu( self.bn9(self.fc9(o8)) ) + self.h_o9(h)
        o10 = self.relu( self.bn10(self.fc10(o9)) ) + self.h_o10(h)
        o11 = self.head( o10 )

        if self.sigmoid:
            return torch.sigmoid(o11.reshape([N, -1]))
        else:
            return o11.reshape([N,-1])